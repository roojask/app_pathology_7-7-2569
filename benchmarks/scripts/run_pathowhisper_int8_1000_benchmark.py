...
def run_pathowhisper_int8_1000_benchmark():
    print("=" * 100, flush=True)
    print("STARTING PATHOWHISPER INT8 ENGINE SCIENTIFIC BENCHMARK EVALUATION (1,000 AUDIO CASES)", flush=True)
    print("=" * 100, flush=True)
    ...
    for i in range(1, 1001):
        ...
        # Audio Pipeline via PathoWhisper INT8 Engine
        case_start = time.time()
        stt_text = ""
        lang_param = "th" if cat_id == 9 or gt.get("accent") == "th" else "en"
        try:
            denoised_path = denoise_audio(audio_path)
            stt_text = transcribe_faster_whisper(denoised_path, language=lang_param)
        except Exception:
            try:
                stt_text = transcribe_faster_whisper(audio_path, language=lang_param)
            except Exception:
                stt_text = ""
        ...
        print(f"{case_id:<12} | {cat_name:<30} | {latency:>10.2f}s | {wer:>8.2f}% | {cer:>8.2f}% | {mapping_acc:>14.2f}%", flush=True)
    ...
    print("\n" + "=" * 100, flush=True)
    print("TABLE 1: HIGH-LEVEL ACADEMIC KPI SUMMARY TABLE (PATHOWHISPER INT8 ENGINE)", flush=True)
    print("=" * 100, flush=True)
    ...
    print("\n" + "=" * 100, flush=True)
    print("TABLE 2: CATEGORY BENCHMARK SUMMARY TABLE (10 CATEGORIES - PATHOWHISPER INT8 ENGINE)", flush=True)
    print("=" * 100, flush=True)
    ...
    for cat_id in range(1, 11):
        ...
        print(f"{cat_id:<8} | {c_name:<32} | {c_lat:>14.2f}s | {c_wer:>8.2f}% | {c_cer:>8.2f}% | {c_acc:>14.2f}%", flush=True)

    print("=" * 100, flush=True)
    print(f"Total Benchmark Runtime: {total_runtime} seconds", flush=True)

if __name__ == "__main__":
    run_pathowhisper_int8_1000_benchmark()
